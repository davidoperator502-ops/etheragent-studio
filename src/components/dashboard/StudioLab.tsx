...
import { Sparkles, Zap, Loader2, Upload } from 'lucide-react';
...
export default function StudioLab() {
    ...
    // 1. Fetch Inicial con Nuevos IDs
    useEffect(() => {
        const fetchAsset = async () => {
            const { data } = await supabase
                .from('visual_assets')
                .select('id, url')
                .in('id', ['studio_new_image', 'studio_new_video']);

            if (data && data.length > 0) {
                const videoAsset = data.find(item => item.id === 'studio_new_video');
                const imgAsset = data.find(item => item.id === 'studio_new_image');
    ...
    // 2. Subida Directa Segura a 'visual-assets'
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        try {
            ...
            const assetId = isVid ? 'studio_new_video' : 'studio_new_image';
    ...
    return (
        <div className="flex flex-col xl:flex-row min-h-screen w-full bg-[#050505] text-white p-4 md:p-8 gap-8 pb-32">
                ...
                <div className="flex-1 bg-[#0a0a0c] border border-white/5 rounded-3xl p-8 flex flex-col relative overflow-visible">
                    ...
                    <div className="bg-zinc-900/50 border border-white/10 p-6 rounded-2xl rounded-tl-sm w-[90%] mb-6">
                        <p className="text-base text-zinc-300 leading-relaxed">
                            He preparado la estructura visual para el comercial de conversión directa. ¿Autorizas inyectar el asset final para revisión ejecutiva?
                        </p>
                    </div>
                    ...
                    {chatStep === 2 && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-zinc-900/50 border border-purple-500/30 p-6 rounded-2xl rounded-tl-sm w-[90%] mb-6">
                            ...
                            <p className="text-base text-white leading-relaxed font-medium">
                                Asset comercial inyectado. Verifica el dispositivo físico a tu derecha.
                            </p>
                        </motion.div>
                    )}
            ...
            {/* PANEL DERECHO: EL MÓVIL PERFECTO (340x720px) */}
            <div className="w-full xl:w-[420px] shrink-0 flex flex-col items-center pt-8">
                ...
                {/* MÓVIL BLINDADO CONTRA APLASTAMIENTO */}
                <div className="relative w-[340px] h-[720px] bg-black border-[8px] border-[#1c1c1e] rounded-[3.5rem] shadow-[0_0_50px_rgba(168,85,247,0.15)] overflow-hidden flex flex-col shrink-0 group">
                    ...
                    <div className="flex-1 relative bg-zinc-900 w-full h-full">
                        {loading ? (
                            ...
                            <div className="absolute inset-0 flex items-center justify-center text-zinc-600 font-mono text-[10px] uppercase text-center p-8 z-10 border border-dashed border-zinc-800 m-4 rounded-xl">
                                Haz clic en "Subir Asset" arriba
                            </div>
                        )}
                    </div>
                </div>
